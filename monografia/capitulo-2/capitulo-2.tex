...
%\chapter{Lógica de Descrição $\mathnormal{ALC}$}
...
\subsection{Sintaxe da Lógica de Descrição}
...
C, D 	& descrição de conceitos \\ \hline
...
Esse trabalho é restrito à família ALC, que compreende os conceitos e propriedades atômicas, negação de conceitos, interseção, união, restrições de valor e existencial, \textit{top} (verdade) e \textit{bottom} (absurdo). A tabela 2.2 mostra além de ALC, outras famílias de DL existentes \cite{dlhandbook:2003}.
...
Os axiomas são compostos por elementos de $N_O$ e um conjunto finito de GCIs (\textit{general concept inclusions}). Podem assumir a forma $C \sqsubseteq D$ ou $C \equiv D$ (uma equivalência ($\equiv$) é o mesmo que $(C \sqsubseteq D) \land (D \sqsubseteq C)$ ), onde $C, D$ são conceitos e $\sqsubseteq$ é uma inclusão.
...
Relação & $R$ & $R^\iota \subseteq \Delta^\iota x \Delta^\iota$ & $AL$ \\ \hline

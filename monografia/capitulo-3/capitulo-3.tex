...
A próxima seção desta monografia descreve o algoritmo proposto pelo orientando para a normalização para a forma normal positiva.
...
As impurezas são identificadas através dos métodos \textit{visit}. A primeira chamada para esse conjunto de métodos é na linha 3 do método \textit{purity}, caso a expressão de entrada seja uma conjunção não pura, ou na linha 5 do método \textit{purify} caso a expressão seja uma disjunção não pura. O método \textit{visit} em sua implementação em Java \footnote{código disponível em: http://github.com/adrianomelo/tg} utiliza o padrão de projetos visitor, que permite criar novas operações sem mudar a definição das estruturas de dados \cite{Gamma:1995}.
...
Quatro parâmetros são passadados para os métodos, uma pilha \textit{stack} que é usada para auxiliar a construção de uma nova expressão normalizada (os métodos não podem retornar objetos por uma limitação da OWLAPI 3.0, biblioteca usada na implementação em Java). Uma expressão em lógica de descrições, que pode ser uma união $\bigcup \limits^n e_i$, interseção $\bigcap \limits^n e_i$, restrição  de valor $\forall r E$, restrição existencial $\exists r E$, complemento $\neg e$ ou conceito $C$. O lado \textit{side} em que a expressão original estava (\textit{left} ou \textit{right}). E o tipo \textit{kind} de expressão que o método estava esperando.

% !TEX encoding = ISO-8859-1
\documentclass[bsc]{ufpethesis}

\usepackage{hyperref}
\hypersetup{
    colorlinks,
    citecolor=red,
    filecolor=red,
    linkcolor=red,
    urlcolor=red
}

%\university{<NOME DA UNIVERSIDADE>}
%\address{<CIDADE DA IES>}
\institute{Centro de Informática}
%\department{Centro de Informática}
\program{Graduação em Ciência da Computação}
\majorfield{Ciência da Computação}
\title{Normalização de ontologias ALC para o uso com o LeanCop}
\date{11 de julho de 2011}
\author{Adriano Silva Tavares de Melo}
\adviser{Frederico Freitas}
%\coadviser{NOME DO(DA) CO-ORIENTADOR(A)}

\begin{document}

%% Parte pré-textual
\frontmatter
% Folha de rosto
\frontpage
% Portada (apresentação)
\presentationpage
% Dedicatória
\begin{dedicatory}
Eu dedico esse trabalho a minha mãe, namorada, familiares e amigos.
\end{dedicatory}
% Agradecimentos
\acknowledgements
\input{../agradecimentos/agradecimentos}

\begin{epigraph}[<NOTA>]{<AUTOR>}
<DIGITE AQUI A CITAÇÂO>
\end{epigraph}

\resumo
A World Wide Web revolucionou a comunicação entre as pessoas em todo o mundo. Com ela, o custo de enviar uma informação de qualquer lugar do mundo para qualquer lugar do mundo foi reduzida a praticamente a zero. Essa facilidade de acesso a informação global fez com que ela crescesse de forma exponencial. 
\begin{keywords}
método de conexões, lógica de descrição, normalização
\end{keywords}

\abstract
\begin{keywords}
<DIGITE AS PALAVRAS-CHAVE AQUI>
\end{keywords}

\tableofcontents
\listoffigures
\listoftables

%% Parte textual
\mainmatter

\include{introducao/introducao}
%\include{capitulo-1/capitulo-1}
\include{capitulo-2/capitulo-2}
\include{capitulo-3/capitulo-3}
\include{capitulo-4/capitulo-4}
\include{conclusao/conclusao}

%% Parte pós-textual
\backmatter

\appendix
% \include{apendice1}

% Bibliografia
% É aconselhável utilizar o BibTeX a partir de um arquivo, digamos "biblio.bib".
% Para ajuda na criaç?o do arquivo .bib e utilizaç?o do BibTeX, recorra ao
% BibTeXpress em www.cin.ufpe.br/~paguso/bibtexpress
\nocite{*}
\bibliographystyle{alpha}
\bibliography{../referencias/referencias}

% Inclui uma pequena nota com referência à UFPEThesis
% \colophon

%% Fim do documento
\end{document}

 ...
 * -----------------------
 * Filename           $RCSfile: OWLTestExtracter.java,v $
 * Revision           $Revision: 1.2 $
 * Release status     $State: Exp $
 * Last modified on   $Date: 2003/11/12 11:06:44 $
...
public class OWLTestExtracter implements RDFConsumer {

    static Logger logger = Logger.getLogger(OWLTestExtracter.class);
 ...
 */
	...
	System.out.println( "Usage: OWLTestExtracter options* url" );
	System.out.println( "  -h HTML output" );
	System.out.println( "  -d Do not include tests with datatypes" );
	System.out.println( "  -p Include proposed tests" );
	System.out.println( "  -x Include extra credit tests" );
	...
	try {
	    ...
	    int c;
	    String arg = null;
	    Getopt g = new Getopt("OWLTestExtracter", args, "?dhl:t:opxz");
...
// 	    System.exit(0);
	    ...
	    OWLTestExtracter extracter = new OWLTestExtracter();
		...
		if ( ( requiredType==null || type.equals( requiredType ) )  
			...
			if ( (obsolete || !otest.getStatus().equals("OBSOLETED") )  
			     &&
			     (proposed || !otest.getStatus().equals("PROPOSED") )  
			     && 
			     (extraCredit || !otest.getStatus().equals("EXTRACREDIT") )  
			     && 
